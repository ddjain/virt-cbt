...
| Field | Value |
|---|---|
| **Scenario ID** | `CBT-V3-09` |
| **Priority** | P3 |
| **Krkn scenario** | `node-cpu-hog | node-memory-hog | node-io-hog` |
| **Target component** | Worker node hosting virt-launcher (one resource class per run) |
| **Inject window** | `T-PROG` — under guest write load during Push copy |
| **Blast radius** | Medium |
| **Folder** | `docs/chaos-test/scenario/cbt-09-node-resource-hog/` |

## 2. Objective

See whether node pressure causes OOM/throttle/checkpoint loss versus clean backup completion or explicit failure.

## 3. Traceability

- Catalog: `docs/chaos-test/chaos_scenario_v3.md` → `CBT-V3-09`
...
- Krkn docs (local): `../../krkn-chaos/website/content/en/docs/scenarios/hog-scenarios/`
...
| Item | Detail |
|---|---|
| **Component under test** | Worker node hosting virt-launcher (one resource class per run) |
| **Why this component** | QEMU bitmaps and Push I/O compete for CPU/mem/disk on the VMI node. |
| **When to inject** | `T-PROG` — under guest write load during Push copy |
| **When NOT to inject** | Hogging control-plane nodes; running all three hogs together. |
| **Backup modes to repeat** | Full and Incremental @ T-PROG; one hog type each |

## 5. Risk / blast radius

Stress one node. Cap below eviction threshold. One of CPU, memory, or I/O per experiment.
...
7. `krknctl run node-cpu-hog --help` reviewed for this Krkn release.
...
| Resolved target | `kubernetes.io/hostname=$VMI_NODE` |
...
Progressing=True; pick one hog scenario.
...
# CPU example — keep percentage moderate on first run
...
# Memory example:
# krknctl run node-memory-hog --node-selector "kubernetes.io/hostname=${VMI_NODE}" \
#   --memory-consumption 70% --chaos-duration 120 --number-of-nodes 1

# I/O example:
# krknctl run node-io-hog --node-selector "kubernetes.io/hostname=${VMI_NODE}" \
#   --chaos-duration 120 --number-of-nodes 1
```
...
1. Hog pod(s) scheduled on VMI node only.
2. Node metrics show intended pressure type (CPU/mem/I/O).
3. Progressing=True at start; no unintended node cordon/eviction storm (or document if it happens).
4. Hog ends after duration; pressure returns toward baseline.
...
1. VMB completes or fails cleanly (no silent success).
2. No unaccounted OOMKill of launcher without VMB failure reflection.
3. Follow-up backup + `cbt-evidence` after pressure removed.
4. Record backup duration vs baseline.
...
Delete leftover hog pods if any; node Ready; V3-11 if needed.

Then run **CBT-V3-11** if the VMB was interrupted or stuck.

## 9. Expected results

Node stable enough for deterministic VMB terminal; evidence-correct follow-up; no unexplained bitmap loss.
...
- Cap below eviction.\n- Never hog masters for this campaign.
